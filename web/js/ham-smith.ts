...
import type { NormalizedZ, Gamma, PixelCoord } from './types.js';
...
const R_CIRCLES: number[] = [0, 0.2, 0.5, 1, 2, 5];    // 归一化电阻圆
const X_CIRCLES: number[] = [0, 0.2, 0.5, 1, 2, 5];    // 归一化电抗圆

// 当前标记点
let markerZ: NormalizedZ | null = null;  // { r: normalizedR, x: normalizedX }
let z0 = 50;         // 特性阻抗
 ...
 * Smith圆图: Γ = (z-1)/(z+1)
 */
function zToGamma(r: number, x: number): Gamma {
 ...
 * Γ → Canvas像素坐标
 */
function gammaToPixel(gr: number, gi: number): PixelCoord {
  return {
 ...
 * Canvas像素 → Γ
 */
function pixelToGamma(px: number, py: number): Gamma {
  return {
 ...
 * Γ → 归一化阻抗 z
 */
function gammaToZ(gr: number, gi: number): NormalizedZ {
  const d = (1 - gr) * (1 - gr) + gi * gi;
  if (d < 1e-12) return { r: Infinity, x: 0 };
 ...
 * 在|Γ|≤1区域内绘制圆弧
 */
function drawArcInUnitCircle(cx_n: number, cy_n: number, cr_n: number, isPositive: boolean): void {
  if (!ctx) return;
  // 计算单位圆与该圆的交点
  const dist = Math.sqrt(cx_n * cx_n + cy_n * cy_n);
  if (dist > cr_n + 1 + 0.001) return; // 不相交
 ...
 * 计算SWR
 */
function calcSWR(gr: number, gi: number): number {
...
function onCanvasMouseMove(_e: MouseEvent): void {
  // 可选：hover预览
}
...
function smithCalc(): void {
  const rVal = parseFloat((document.getElementById('smithR') as HTMLInputElement).value);
  const xVal = parseFloat((document.getElementById('smithX') as HTMLInputElement).value);
  const z0Val = parseFloat((document.getElementById('smithZ0') as HTMLInputElement).value);
...
function smithClear(): void {
  markerZ = null;
  (document.getElementById('smithR') as HTMLInputElement).value = '';
  (document.getElementById('smithX') as HTMLInputElement).value = '';
  (document.getElementById('smithZ0') as HTMLInputElement).value = '50';
  (document.getElementById('smithResult') as HTMLElement).textContent = '';
  drawSmithChart();
}

function updateInputFields(): void {
  if (!markerZ) return;
  (document.getElementById('smithR') as HTMLInputElement).value = (markerZ.r * z0).toFixed(2);
  (document.getElementById('smithX') as HTMLInputElement).value = (markerZ.x * z0).toFixed(2);
  (document.getElementById('smithZ0') as HTMLInputElement).value = String(z0);
  updateResultText();
}
...
function showResult(txt: string): void {
...
function init(): void {
  canvas = document.getElementById('smithCanvas') as HTMLCanvasElement | null;
  if (!canvas) return;
