 ...
 * @returns 'wtia' | 'n2yo'
 */
export function getApiSource(): SatApiSource {
  try {
    return (localStorage.getItem(API_SOURCE_STORAGE) as SatApiSource) || 'wtia';
  } catch (e) {
    return 'wtia';
  }
}
 ...
 */
export function setApiSource(source: SatApiSource): void {
  try {
 ...
 * @returns N2YO位置数据
 */
export async function fetchSatPosition(noradId: number, obsLat: number, obsLon: number, obsAlt: number = 0, seconds: number = 0): Promise<N2yoApiResponse | null> {
  ...
  try {
    ...
    const data: N2yoApiResponse = await resp.json();
    if (data.error) {
 ...
 * @returns N2YO过境数据
 */
export async function fetchSatPasses(noradId: number, obsLat: number, obsLon: number, obsAlt: number = 0, days: number = 1, minElevation: number = 10): Promise<N2yoApiResponse | null> {
  const apiKey = getApiKey();
  if (!apiKey) return null;
  try {
 ...
 */
export function parseN2yoPosition(n2yoData: N2yoApiResponse | null): N2yoPosition | null {
  if (!n2yoData || !n2yoData.positions || !n2yoData.positions.length) return null;
 ...
 * @returns 过境信息数组
 */
export function parseN2yoPasses(n2yoData: N2yoApiResponse | null): SatPass[] {
  if (!n2yoData || !n2yoData.passes) return [];
 ...
 * @returns WTIA位置数据
 */
export async function fetchWtiaPosition(noradId: number): Promise<WtiaApiResponse | null> {
  try {
    ...
    const data: WtiaApiResponse = await resp.json();
    if (data.error) {
 ...
 */
export function parseWtiaPosition(wtiaData: WtiaApiResponse | null): WtiaPosition | null {
  if (!wtiaData || !wtiaData.latitude) return null;
  return {
 ...
 * @param noradId - NORAD编号
 */
export function isWtiaSupported(noradId: number): boolean {
 ...
 * @returns 卫星信息数组
 */
export function getPopularSatellites(): SatelliteInfo[] {
  return [
...
// ============================================================
// 仰角/方位角几何计算（WTIA数据需要，纯几何无轨道模型误差）
// ============================================================
 ...
 * @returns 仰角、方位角、距离
 */
export function calcLookAngle(satPos: SatPositionInput, obsLat: number, obsLon: number, obsAlt: number = 0): LookAngle {
  ...
  let xSat: number, ySat: number, zSat: number;
  if (satPos.xEcef !== undefined) {
    xSat = satPos.xEcef; ySat = satPos.yEcef!; zSat = satPos.zEcef!;
  } else {
