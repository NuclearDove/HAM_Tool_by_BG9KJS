 ...
 * @returns {Promise<Array<{name: string, noradId: number}>>} 按名称排序的卫星列表
 */
 ...
 * @param {boolean} forceRefresh - 是否强制刷新（忽略内存缓存）
 * @returns {Object|null} { tles: {noradId: {name,line1,line2}}, fetchTime, fromCache }
 */
async function fetchTleData(forceRefresh = false) {
    // 使用内存缓存（非强制刷新时）
    if (!forceRefresh && tleMemoryCache) {
 ...
 * @param {string} text - Celestrak返回的TLE文本
 * @returns {Object} { noradId: { name, line1, line2 } }
 */
 ...
 * @returns {Object|null} { tles, fetchTime }
 */
 ...
 * @param {number} noradId - NORAD编号
 * @returns {Object|null} { name, line1, line2, fetchTime, fromCache, epoch }
 */
 ...
 * @returns {Object|null} { lat, lon, alt, vel, timestamp } 或 null
 */
export function calcSgp4Position(line1, line2, date) {
    if (typeof satellite === 'undefined') {
        console.warn('[HAM] satellite.js库未加载，无法进行轨道计算');
        return null;
    }
    try {
...
/**
 * 格式化TLE数据年龄为可读字符串
 * @param {Date} epoch - TLE历元时间
 * @returns {string} 如 "3天前"、"2小时前"
 */
export function formatTleAge(epoch) {
    ...
    if (ageHours < 1) return '不到1小时';
    if (ageHours < 24) return ageHours + '小时';
    const ageDays = Math.floor(ageHours / 24);
    if (ageDays === 1) return '1天';
    return ageDays + '天';
}
 ...
 * 用于TLE过境预测等需要高精度的场景
 * @param {Object} posEci - satellite.js propagate返回的ECI位置 {x,y,z}
 * @param {number} gmst - 格林尼治恒星时(弧度)
 * @param {number} obsLat - 观测者纬度(度)
 * @param {number} obsLon - 观测者经度(度)
 * @param {number} obsAlt - 观测者海拔(km)
 * @returns {{elevation:number, azimuth:number, distance:number}}
 */
 ...
 * @param {number} obsLat - 观测者纬度（度）
 * @param {number} obsLon - 观测者经度（度）
 * @param {number} [obsAlt=0] - 观测者海拔（km）
 * @param {number} [durationHours=24] - 预测时长（小时）
 * @param {number} [minElev=10] - 最小仰角阈值（度）
 * @param {number} [stepSeconds=30] - 采样间隔（秒）
 * @returns {Promise<Array>} 过境数组 [{start,peak,end,maxElevation,azimuth,startAz,endAz}]
 */
export async function predictPassesLocal(noradId, obsLat, obsLon, obsAlt = 0, durationHours = 24, minElev = 10, stepSeconds = 30) {
    if (typeof satellite === 'undefined') {
        console.warn('[HAM] satellite.js库未加载，无法进行过境预测');
        return [];
    }
    // 获取TLE数据
    ...
    try {
        ...
        for (const coarse of coarsePasses) {
            ...
            // 仅记录达到最小仰角阈值的过境
            if (fineMaxElev >= minElev && fineStart && fineEnd) {
                passes.push({
                    start: fineStart,
                    peak: finePeakTime,
                    end: fineEnd,
