 ...
 */
export function calcModeStats(qsos: QsoEntry[]): [string, number][] {
 ...
 */
export function calcBandStats(qsos: QsoEntry[]): [string, number][] {
  const map: Record<string, number> = {};
 ...
 */
export function calcMonthStats(qsos: QsoEntry[]): MonthStats {
  const now = new Date();
 ...
 * @returns HTML字符串
 */
export function renderStatsTable(title: string, label: string, data: [string, number][], total: number): string {
 ...
 * @returns 完整HTML字符串
 */
export function renderStatsHtml(total: number, uniqueCalls: number, modeStats: [string, number][], bandStats: [string, number][], monthStats: MonthStats): string {
 ...
 * @returns 验证结果
 */
export function validateQso(qso: Partial<QsoEntry>): QsoValidationResult {
  const errors: string[] = [];
  if (!qso.call) errors.push('呼号不能为空');
  if (!qso.freq || Number(qso.freq) <= 0) errors.push('频率必须大于0');
  if (!qso.mode) errors.push('模式不能为空');
  if (!qso.date) errors.push('日期不能为空');
  if (!qso.time) errors.push('时间不能为空');
 ...
 * @returns 唯一呼号数
 */
export function countUniqueCalls(qsos: QsoEntry[]): number {
