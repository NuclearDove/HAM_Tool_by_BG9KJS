 ...
 */
export function latLonToGrid(lat: number, lon: number, precision: number = 6): string {
  let tmp = lon + 180;
 ...
 */
export function calcDistanceBearing(lat1: number, lon1: number, lat2: number, lon2: number): DistanceBearingResult {
  const R = 6371;
 ...
 * @returns 经纬度或null
 */
export function parseQth(qth: string): LatLon | null {
  if (!qth) return null;
