...
import type { MorseCodeMap, MorseDecodeMap } from '../types.js';
...
const MORSE_DECODE: MorseDecodeMap = Object.fromEntries(
...
export function morseDecode(morse: string): string {
  ...
  return morse.trim().split(/\s*\/\s*/).map(word =>
    word.trim().split(/\s+/).map(code => MORSE_DECODE[code] || '').join('')
  ).join(' ');
}
 ...
 * @returns 莫尔斯电码映射对象
 */
export function getMorseTable(): MorseCodeMap {
  return { ...MORSE_CODE };
}