...
let customClocks: ClockConfig[] = [];
try { customClocks = JSON.parse(localStorage.getItem(CONFIG.CLOCK_KEY) || '[]') || []; } catch(_e) {}
 ...
 */
function _buildClockDOM(grid: HTMLElement, all: ClockConfig[]): void {
  ...
  grid.onclick = function(e: MouseEvent) {
    ...
    const delBtn = target.closest('.clock-del-btn') as HTMLElement | null;
    if (delBtn) {
 ...
 */
function _updateClockText(grid: HTMLElement | null, all: ClockConfig[]): void {
  if (!grid) return;
...
const TZ_LIST: string[] = (() => {
  try { const list = (Intl as any).supportedValuesOf('timeZone'); if (list && list.length) return list; } catch(_e) {}
  return [];
})();
...
function _fmtTz(tz: string): string {
...
function filterClockOptions(): void {
  const input = (document.getElementById('customTzInput') as HTMLInputElement).value.trim();
  const dropdown = document.getElementById('tzDropdown') as HTMLElement | null;
  if (!dropdown) return;
  ...
  const seenTz: Record<string, boolean> = {};
  ...
  // 标记已添加
  const all = DEFAULT_CLOCKS.concat(customClocks);
  const addedSet: Record<string, boolean> = {};
...
function hideClockOptions(): void {
  setTimeout(() => {
...
function selectClock(name: string, tz: string): void {
  (document.getElementById('customTzInput') as HTMLInputElement).value = '';
  (document.getElementById('tzDropdown') as HTMLElement).style.display = 'none';
  _doAddClock(name, tz);
}

function _doAddClock(name: string, tz: string): void {
  const all = DEFAULT_CLOCKS.concat(customClocks);
...
function removeClock(idx: number): void {
  const all = DEFAULT_CLOCKS.concat(customClocks);
...
function clearCustomClocks(): void {
...
// 传播条件
function propFetch(): void {
  (document.getElementById('propStatus') as HTMLElement).textContent = '正在刷新...';
  const ts = Date.now();
  ...
  (document.getElementById('propResult') as HTMLElement).innerHTML =
    '<div style="display:flex;flex-wrap:wrap;gap:16px;align-items:flex-start">' + imgHtml + h + '</div>';
  (document.getElementById('propStatus') as HTMLElement).textContent = '已更新';
}

// RST 信号报告
function rstQuery(): void {
  const input = (document.getElementById('rstInput') as HTMLInputElement).value.trim();
  if (!input) { (document.getElementById('rstResult') as HTMLElement).textContent = '请输入RST值。'; return; }
  ...
  (document.getElementById('rstResult') as HTMLElement).textContent = lines.join('\n');
}
 ...
 */
function clockLoop(timestamp: number): void {
...
function init(): void {
  ...
  if (dropdown) {
    dropdown.addEventListener('mousedown', (e: MouseEvent) => {
      const target = e.target as HTMLElement;
      const opt = target.closest('.tz-option') as HTMLElement | null;
      if (opt) {
