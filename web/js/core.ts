...
import type { AppConfig, Band, BandSub, BandChannel, CoaxTypes, CoaxAttenuationMap, ModeRefCategory, QCodeEntry, AbbrevEntry, ClockConfig, LatLon, QthLocation, DistanceBearingResult, DipoleResult, MorseCodeMap } from './types.js';
...
// 频段分类
// ============================================================
const SPECTRUM_CATS: [number, number, string][] = [
...
// 通信模式
// ============================================================
let MODES: string[] = ["SSB","CW","FM","AM","DMR","C4FM","D-STAR","FT8","FT4","RTTY","PSK31","JT65","JT9","WSPR","SSTV","ATV","其他"];

// ============================================================
// Q简语（ITU Appendix 14）
// ============================================================
let Q_CODES: QCodeEntry[] = [
...
const MORSE_REVERSE: MorseCodeMap = {};
Object.entries(MORSE_CODE).forEach(([k, v]) => { MORSE_REVERSE[v] = k; });

// ============================================================
// 城市坐标
// ============================================================
let CITY_COORDS: Record<string, [number, number]> = {
...
// 默认时钟
// ============================================================
let DEFAULT_CLOCKS: ClockConfig[] = [
...
// RST 信号报告
// ============================================================
let R_READABILITY: string[] = ['','不可辨','勉强可辨','可辨但困难','可辨，有困难','完美可辨'];
let S_STRENGTH: string[] = ['','极微弱','很微弱','微弱','弱','较好','好','相当强','强','很强','极强'];
let T_TONE: string[] = ['','粗糙直流','非常粗糙','粗糙低频交流','粗糙交流','带调制交流','调制好，有杂音','接近直流，轻微调制','好的直流，有轻微调制','极好的直流，低杂音','完美直流'];
 ...
 */
function populateSelect<T>(selectId: string, items: T[], valueFn: (item: T) => string, textFn: (item: T) => string): void {
 ...
 */
function renderTable<T>(tableId: string, data: T[], rowFn: (row: T) => string): void {
 ...
 * @returns MHz值
 */
function freqToMHz(val: number | string, unit: string): number {
 ...
 * @returns 频段对象或null
 */
function freqToBand(freqMHz: number): Band | null {
 ...
 */
function getSpectrumCat(freqMHz: number): string | null {
 ...
 * @returns 波长(米)
 */
function wavelength(freqMHz: number): number { return RfService.calcWavelength(freqMHz); }
 ...
 * @returns 半波长度和每臂长度(米)
 */
function dipoleLength(freqMHz: number): DipoleResult { return RfService.calcDipoleLength(freqMHz); }
 ...
 */
function quarterWaveLength(freqMHz: number): number { return RfService.calcQuarterWaveLength(freqMHz); }
 ...
 * @returns 经纬度对象或null
 */
function gridToLatLon(grid: string): LatLon | null { return GeoService.gridToLatLon(grid); }
 ...
 */
function latLonToGrid(lat: number, lon: number): string | null { return GeoService.latLonToGrid(lat, lon); }
 ...
 */
function distanceBearing(lat1: number, lon1: number, lat2: number, lon2: number): DistanceBearingResult {
...
function dbmToWatts(dbm: number): number { return RfService.dbmToWatts(dbm); }
/** @param w - 瓦特 @returns dBm值 */
function wattsToDbm(w: number): number { return RfService.wattsToDbm(w); }
/** @param w - 瓦特 @returns dBW值 */
function wattsToDbw(w: number): number { return RfService.wattsToDbw(w); }
/** @param dbw - dBW值 @returns 瓦特 */
function dbwToWatts(dbw: number): number { return RfService.dbwToWatts(dbw); }
/** @param dbm - dBm值 @returns dBW值 */
function dbmToDbw(dbm: number): number { return RfService.dbmToDbw(dbm); }
 ...
 */
function swrFromPower(fw: number, rw: number): number | null { return RfService.calcSwrFromPower(fw, rw); }
 ...
 */
function swrToReflectPower(swr: number, fw: number): number | null { return RfService.calcReflectPower(swr, fw); }
 ...
 */
function coaxAttenuation(type: string, freqMHz: number, lengthM: number): number | null {
 ...
 * @returns 坐标对象或null
 */
function parseQth(qth: string): QthLocation | null {
  if (!qth) return null;
  qth = qth.trim();
  ...
  if (gridMatch) {
    ...
    if (c) {
      const result: QthLocation = { ...c, source: 'grid', grid: gridMatch[1].toUpperCase() };
      return result;
    }
    return null;
  }
...
async function initData(): Promise<void> {
  try {
    await Data.loadAll();
    if (Data.get('bands') && Data.get('bands').length) BANDS = Data.get('bands');
    if (Data.get('modes') && Data.get('modes').length) MODES = Data.get('modes');
    if (Data.get('coaxTypes') && Object.keys(Data.get('coaxTypes')).length) COAX_TYPES = Data.get('coaxTypes');
    if (Data.get('qCodes') && Data.get('qCodes').length) Q_CODES = Data.get('qCodes');
    if (Data.get('engAbbrev') && Data.get('engAbbrev').length) ENG_ABBREV = Data.get('engAbbrev');
    if (Data.get('numAbbrev') && Data.get('numAbbrev').length) NUM_ABBREV = Data.get('numAbbrev');
    if (Data.get('cityCoords') && Object.keys(Data.get('cityCoords')).length) CITY_COORDS = Data.get('cityCoords');
    if (Data.get('modesRef') && Data.get('modesRef').length) MODES_REF = Data.get('modesRef');
    if (Data.get('defaultClocks') && Data.get('defaultClocks').length) DEFAULT_CLOCKS = Data.get('defaultClocks');
    if (Data.get('rstData')) {
      if (Data.get('rstData').readability) R_READABILITY = Data.get('rstData').readability;
      if (Data.get('rstData').strength) S_STRENGTH = Data.get('rstData').strength;
      if (Data.get('rstData').tone) T_TONE = Data.get('rstData').tone;
    }
...
// ============================================================
export {
  CONFIG, C, CAT_CN, B_CLASS_NOTE, SPECTRUM_CATS,
  MORSE_CODE, MORSE_REVERSE,
  populateSelect, renderTable, escHtml,
  ...
  initData,
  BANDS, COAX_TYPES, MODES, Q_CODES, ENG_ABBREV, NUM_ABBREV,
  CITY_COORDS, MODES_REF, DEFAULT_CLOCKS,
  R_READABILITY, S_STRENGTH, T_TONE,
  RfService, GeoService, MorseService, LogService, VoacapService, SatelliteService
};