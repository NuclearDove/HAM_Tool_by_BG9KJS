...
function freqQuery() {
    ...
    if (isNaN(f1)) {
        document.getElementById('freqResult').textContent = '请输入有效频率值。';
        return;
    }
    const lines = [];
    if (mode === 'freq') {
        ...
        if (isNaN(f2)) {
            document.getElementById('freqResult').textContent = '请输入有效上限频率。';
            return;
        }
...
function initBandTable() {
    ...
    tbody.addEventListener('click', function (e) {
        const row = e.target.closest('tr.band-parent');
        if (!row)
            return;
        const bandName = row.dataset.band;
        const subs = tbody.querySelectorAll('tr[data-parent="' + bandName + '"]');
        const icon = row.querySelector('.toggle-icon');
        if (!icon)
            return;
        const expanded = icon.textContent === '▼';
        subs.forEach((sub) => { sub.style.display = expanded ? 'none' : ''; });
...
//# sourceMappingURL=ham-freq.js.map