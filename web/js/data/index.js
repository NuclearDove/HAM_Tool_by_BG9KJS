 ...
 */
/**
 * 创建5秒超时的AbortSignal（兼容不支持AbortSignal.timeout的浏览器）
 */
function _createTimeoutSignal() {
    if (typeof AbortSignal !== 'undefined' && typeof AbortSignal.timeout === 'function') {
        return AbortSignal.timeout(5000);
    }
    // fallback：手动创建AbortController + setTimeout
    const controller = new AbortController();
    setTimeout(() => controller.abort(), 5000);
    return controller.signal;
}
async function _fetchJson(path) {
    const resp = await fetch(path, { signal: _createTimeoutSignal() });
    if (!resp.ok)
