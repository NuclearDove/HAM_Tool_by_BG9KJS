...
function mapRender() {
    ...
    if (!container)
        return;
    // 检查 MapLibre GL JS 是否可用
    if (typeof maplibregl === 'undefined') {
        console.warn('[HAM Map] maplibregl not available, showing fallback');
    ...
    if (!mapInstance) {
        try {
            ...
            mapInstance.on('load', function () {
                ...
                try { mapInstance.resize(); } catch (_) { /* ignore */ }
            });
        }
        catch (e) {
    ...
    // 清除旧标记
    markers.forEach(function (m) { return m.remove(); });
    markers = [];
    // 等待地图加载完成后添加标记
    var addMarkers = function () {
        // 动态导入Log模块获取数据
        import('./ham-log.js').then(function (_a) {
            var loadQsos = _a.loadQsos;
            var qsos = loadQsos();
            var locs = [];
            qsos.forEach(function (q) {
                if (!q.qth)
                    return;
                var loc = parseQth(q.qth);
                if (!loc)
                    return;
                locs.push(loc);
                // WGS-84 → GCJ-02 坐标转换（高德地图坐标系）
                var gcj = wgs84ToGcj02(loc.lon, loc.lat);
            ...
            if (locs.length > 0) {
                // 计算边界 [minLng, minLat, maxLng, maxLat]（转换后的GCJ-02坐标）
                var gcjLocs = locs.map(function (l) { return wgs84ToGcj02(l.lon, l.lat); });
                var minLng = Math.min.apply(Math, gcjLocs.map(function (l) { return l.lng; }));
                var maxLng = Math.max.apply(Math, gcjLocs.map(function (l) { return l.lng; }));
                var minLat = Math.min.apply(Math, gcjLocs.map(function (l) { return l.lat; }));
                var maxLat = Math.max.apply(Math, gcjLocs.map(function (l) { return l.lat; }));
                var bounds = new maplibregl.LngLatBounds([minLng, minLat], [maxLng, maxLat]);
                mapInstance.fitBounds(bounds, { padding: 30, maxZoom: 12 });
            }
            // 延迟resize确保容器布局稳定
            setTimeout(function () { try { mapInstance.resize(); } catch (_) { /* ignore */ } }, 200);
        }).catch(function (err) {
...
function retryMapLoad() {
    ...
    // 尝试重新加载MapLibre脚本（使用jsDelivr CDN，国内可访问）
    var existingScript = document.querySelector('script[src*="maplibre"]');
    if (existingScript)
        existingScript.remove();
    // 清除旧maplibregl对象和地图实例
    mapInstance = null;
    markers = [];
