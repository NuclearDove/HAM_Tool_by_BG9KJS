...
async function callLookup() {
    ...
    try {
        ...
        if (data && data._error) {
            const errMsg = data._error === 'timeout' ? 'HamQTH查询超时，请稍后重试' : 'HamQTH查询失败，请检查网络连接';
            const localData = lookupLocalPrefix(call);
            if (localData) {
                renderLocalResult(call, localData, resultEl);
                if (statusEl)
                    statusEl.textContent = 'API不可用，本地查询';
 ...
 * 从HamQTH API查询
 */
async function fetchHamQTH(call) {
    try {
        ...
        return { _error: isTimeout ? 'timeout' : 'network' };
    }
}
/**
 * 解析HamQTH XML响应
 */
function parseHamQTHXml(text) {
    try {
 ...
 * 本地前缀查询
 */
function lookupLocalPrefix(call) {
    // 尝试2字符前缀，然后1字符
    const prefix2 = call.substring(0, 2);
    const prefix1 = call.substring(0, 1);
    if (CALL_PREFIXES[prefix2])
        return CALL_PREFIXES[prefix2];
    if (CALL_PREFIXES[prefix1])
        return CALL_PREFIXES[prefix1];
    // 尝试数字前缀（如7J, 4X等）
    const prefixNum = call.match(/^[A-Z0-9]{1,2}[0-9]/);
    if (prefixNum) {
        const p = prefixNum[0].substring(0, prefixNum[0].length - 1);
        if (CALL_PREFIXES[p])
            return CALL_PREFIXES[p];
    }
    return null;
}
/**
 * 渲染在线查询结果
 */
function renderCallResult(data, el) {
    let html = '<table class="data-table" style="text-align:left">';
    html += '<tr><th style="width:120px">呼号</th><td><b>' + escHtml(data.call) + '</b></td></tr>';
    if (data.name)
        html += '<tr><th>姓名</th><td>' + escHtml(data.name) + '</td></tr>';
    if (data.address)
        html += '<tr><th>地址</th><td>' + escHtml(data.address) + '</td></tr>';
    if (data.city)
        html += '<tr><th>城市</th><td>' + escHtml(data.city) + '</td></tr>';
    if (data.country)
        html += '<tr><th>国家</th><td>' + escHtml(data.country) + '</td></tr>';
    if (data.grid)
        html += '<tr><th>网格</th><td>' + escHtml(data.grid) + '</td></tr>';
    if (data.lat && data.lon)
        html += '<tr><th>坐标</th><td>' + escHtml(String(data.lat)) + ', ' + escHtml(String(data.lon)) + '</td></tr>';
    if (data.qsl)
        html += '<tr><th>QSL</th><td>' + escHtml(data.qsl) + '</td></tr>';
    if (data.url)
        html += '<tr><th>主页</th><td><a href="' + escHtml(data.url) + '" target="_blank">' + escHtml(data.url) + '</a></td></tr>';
 ...
 * 渲染本地查询结果
 */
function renderLocalResult(call, data, el) {
    let html = '<table class="data-table" style="text-align:left">';
...
//# sourceMappingURL=ham-callsign.js.map