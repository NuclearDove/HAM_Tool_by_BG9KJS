...
import type { QsoEntry, QthLocation } from './types.js';

// MapLibre GL JS全局变量声明
 ...
 * @returns GCJ-02坐标
 */
function wgs84ToGcj02(lng: number, lat: number): Gcj02Coord {
...
function mapRender(): void {
  ...
  const addMarkers = (): void => {
    // 动态导入Log模块获取数据
    import('./ham-log.js').then(({ loadQsos }: { loadQsos: () => QsoEntry[] }) => {
      const qsos = loadQsos();
      const locs: QthLocation[] = [];
      qsos.forEach(q => {
        if (!q.qth) return;
        ...
        const marker = new maplibregl.Marker()
          ...
          .setPopup(new maplibregl.Popup({ offset: 25 }).setHTML(
            '<b>' + q.call + '</b><br>' + q.date + ' ' + q.time + '<br>' +
      ...
      if (countEl) countEl.textContent = markers.length + ' 个标记';
      if (locs.length > 0) {
        // 计算边界（转换后的GCJ-02坐标）
      ...
      // 延迟resize确保容器布局稳定
      setTimeout(() => { try { mapInstance.resize(); } catch (_) { /* ignore */ } }, 200);
    }).catch(err => {
...
function renderFallback(container: HTMLElement): void {
  import('./ham-log.js').then(({ loadQsos }: { loadQsos: () => QsoEntry[] }) => {
    ...
    if (qsos.length === 0) {
      ...
      html += '</table>';
      const countEl = document.getElementById('mapCount');
      if (countEl) countEl.textContent = count + ' 个坐标';
    }
    html += '</div>';
...
function retryMapLoad(): void {
  ...
  const existingScript = document.querySelector('script[src*="maplibre"]');
  if (existingScript) existingScript.remove();
  // 清除旧maplibregl对象和地图实例
  mapInstance = null;
  markers = [];
  mapInitialized = false;
  if (typeof maplibregl !== 'undefined') delete (window as any).maplibregl;
...
function init(): void { /* 地图在Tab切换时按需初始化 */ }
